...
class RiskManager {
     ...
     */
    calculatePositionSize(winRate, odds, volatility) {
     ...
     */
    calculateUsdPositionSize(winRate, odds, volatility) {
     ...
     */
    getDynamicStopLoss(entryPrice, volatility) {
    ...
    calculatePortfolioRisk() {
        const totalExposure = this.portfolio.positions.reduce((sum, pos) => sum + pos.valueUsd, 0);
     ...
     */
    updatePortfolioPnL(pnl, isWin) {
        this.portfolio.dailyPnL += pnl;
        this.portfolio.totalPnL += pnl;
        // Update win rate
        const totalTrades = this.portfolio.positions.length;
        if (totalTrades > 0) {
            const wins = this.portfolio.positions.filter(p => (p.valueUsd - p.entryTime.getTime()) > 0).length;
