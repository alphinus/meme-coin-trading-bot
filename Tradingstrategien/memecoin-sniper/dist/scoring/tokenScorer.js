...
class TokenScorer {
     ...
     */
    async scoreToken(token, mlProbability, sentimentScore) {
        const factors = [];
    ...
    calculateLiquidityScore(token) {
        ...
        if (token.liquidity >= goodLiquidity)
            return 100;
    ...
    calculateSecurityScore(token) {
        ...
        if (token.isFreezeRevoked)
            score += 15;
        // Liquidity burned
        if (token.isLiquidityBurned)
            score += 10;
        return Math.min(score, 100);
    }
    async calculateSocialScore(token) {
        let score = 50; // Base score
    ...
    calculateRiskScore(token, factors) {
        ...
        if (liquidityFactor && liquidityFactor.value < 30)
            risk += 15;
        // No social presence
        const socialFactor = factors.find((f) => f.name === 'social');
        if (socialFactor && socialFactor.value < 30)
            risk += 10;
...
exports.TokenScorer = TokenScorer;
