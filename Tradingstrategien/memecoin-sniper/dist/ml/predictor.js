...
class MLPredictor {
     ...
     */
    async predict(token, liveFeatures) {
        if (!this.modelLoaded) {
     ...
     */
    extractFeatures(token, liveFeatures) {
    ...
    calculateConfidence(features) {
        ...
        let dataPoints = 0;
        let totalPoints = 10;
        if (features.liquidity_ratio > 0)
            dataPoints++;
        if (features.holder_concentration > 0)
            dataPoints++;
        if (features.buy_sell_ratio_1min !== 1.0)
            dataPoints++;
        if (features.volume_acceleration > 0)
            dataPoints++;
        if (features.tweet_velocity > 0)
            dataPoints++;
        if (features.sentiment_score !== 0.5)
            dataPoints++;
        if (features.influencer_mentions > 0)
            dataPoints++;
        if (features.community_growth > 0)
            dataPoints++;
        if (features.mint_revoked > 0)
            dataPoints++;
        if (features.liquidity_burned > 0)
            dataPoints++;
