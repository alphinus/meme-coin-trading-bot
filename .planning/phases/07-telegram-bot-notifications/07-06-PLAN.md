...
<task type="auto">
  ...
  <action>
    ...
    **File 2: `server/src/routes/ideas.ts`**
    ...
    2. Add `emitNotificationForEvent(event)` call after every existing `evaluateTriggersForEvent(event)` call. There are 7 locations (lines ~105-106, ~123-124, ~292-293, ~398-399, ~478-479, ~502-503, ~574-575). After each `evaluateTriggersForEvent(...)` line, add the corresponding `emitNotificationForEvent(...)` call.

    **File 3: `server/src/routes/ai.ts`**

    Skip ai.ts — no changes needed. The events emitted in ai.ts routes (ai.member_configured, ai.message_read, ai.model_overridden, ai.decision_overridden, ai.mediation_requested) are NOT in the EVENT_NOTIFICATION_MAP in emitter.ts. The mapped AI events (ai.trigger_fired, ai.mediation_completed) are emitted from the AI trigger engine and mediation module, not from route handlers. No import or calls needed.
  ...
  <verify>
    1. Run `npx tsc --noEmit` from server directory -- no new type errors
    2. Grep all modified files for "emitNotificationForEvent":
       - `grep -r "emitNotificationForEvent" server/src/routes/projects.ts server/src/routes/ideas.ts` should show imports and calls
       - `grep "emitNotificationForEvent" server/src/workflow/engine.ts` should show import and calls
    3. Count total emitNotificationForEvent calls (excluding imports): should be >= 13 calls across all 3 files
  </verify>
  <done>
...
- emitNotificationForEvent is imported and called in projects.ts, ideas.ts, and workflow/engine.ts
