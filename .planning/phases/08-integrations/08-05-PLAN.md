...
<task type="auto">
  ...
  <action>
**0. Investigate AI SDK API surface (critical -- resolves API uncertainty):**
   - Read `server/node_modules/ai/package.json` to confirm installed version.
   - Read `server/node_modules/ai/dist/index.d.ts` (or check the main entry point type declarations) to confirm:
     a. Does `tool()` use `parameters` or `inputSchema` for the Zod schema property? Check the ToolDefinition type.
     b. Does `generateText()` accept `maxSteps` or use `stopWhen: stepCountIs(N)` for multi-step tool use?
     c. What does the `result` object look like? Does it have `result.text`, `result.steps[].toolCalls`, `result.toolCalls`?
   - Also check how the existing codebase uses AI SDK: `grep -r "from \"ai\"" server/src/` to find existing import patterns and API usage.
   - Document findings in a code comment at the top of tools.ts for future reference.
   - Use the confirmed API conventions throughout this plan (do NOT guess).

**Then create `server/src/search/tools.ts`:**
...
Define and export `searchTools` object with these tools (use the property name confirmed in step 0 -- `parameters` or `inputSchema`):
...
4. `search_ideas`: tool({
  ...
  <done>
    AI SDK API surface confirmed (parameters vs inputSchema, maxSteps vs stopWhen). Four search tools defined with Zod schemas: search_projects (name/phase/archived), search_decisions (project decisions and milestones), search_events (broad event search), search_ideas (idea pool search). Each tool queries the JSONL event store via existing reducers and readEvents. TypeScript compiles.
  </done>
</task>
...
1. Create `server/src/search/engine.ts`:
   ...
   c. Call generateText with tools and multi-step option (use the API convention confirmed in Task 1 step 0 -- either `maxSteps: 5` or the older `stopWhen` pattern, whichever was confirmed).
      Example: generateText({ model, tools: searchTools, maxSteps: 5, system: systemPrompt, prompt: query })
   ...
   e. Extract sources from tool calls: result.steps.flatMap(s => s.toolCalls || []).map(tc => `${tc.toolName}(${JSON.stringify(tc.args)})`)
      (Adjust property names based on confirmed API shape from Task 1 step 0)
...
2. Create `server/src/routes/search.ts`:
  ...
  <verify>
    ...
    Grep for "maxSteps" or "stopWhen" in engine.ts (verify multi-step tool use is configured -- use whichever was confirmed in Task 1).
...
- AI search uses tool-calling with multi-step enabled (maxSteps or equivalent confirmed from API investigation)
