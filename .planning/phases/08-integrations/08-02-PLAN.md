...
must_haves:
  ...
  key_links:
    ...
    - from: "server/src/integrations/google/gmail.ts"
      to: "server/src/integrations/email/router.ts"
      via: "pollGmailInbox calls analyzeEmailRouting after emitting email_received"
      pattern: "analyzeEmailRouting"
    - from: "server/src/integrations/google/gmail.ts"
      to: "server/src/events/store.ts"
      via: "appendEvent for integration.email_received, integration.email_routed, and integration.gmail_synced"
...
<task type="auto">
  <name>Task 1: Email parser and Gmail polling with AI routing wiring</name>
  <files>
    server/src/integrations/email/parser.ts
    server/src/integrations/google/gmail.ts
  </files>
...
2. Create `server/src/integrations/google/gmail.ts`:
   ...
   - Import { analyzeEmailRouting } from "../email/router.js"
   ...
   - `pollGmailInbox(userId: string, teamId: string, language: "de" | "en")`: Follows the research pattern exactly:
     ...
     e. For each new message: fetch with gmail.users.messages.get({ userId: "me", id, format: "metadata", metadataHeaders: ["From", "To", "Subject", "Date"] }). Then emit integration.email_received event with { messageId, threadId, from, to, subject, snippet, receivedAt }. Fire-and-forget: emitNotificationForEvent on the email_received event.
     f. **AI routing (fire-and-forget):** After emitting email_received for each message, fetch full body via fetchFullMessage(messageId), then call analyzeEmailRouting(subject, body, from, teamId, language). Emit integration.email_routed event with { messageId, suggestions: routingResult }. This runs per-message but does NOT block the polling loop -- wrap in a Promise that is not awaited (fire-and-forget).
     g. Update historyId checkpoint via integration.gmail_synced event
     h. Wrap entire function in try/catch, log errors but do not throw (polling must be resilient)
   ...
   - `startGmailPolling(userId: string, teamId: string, language: "de" | "en")`: setInterval calling pollGmailInbox every 60_000ms (60 seconds). Store interval handle in module-level variable.
   - `stopGmailPolling()`: clearInterval on stored handle.
  </action>
  <verify>
    Run `cd /Users/developer/eluma && npx tsc --noEmit` -- zero errors.
    Grep for "analyzeEmailRouting" in gmail.ts to confirm AI routing is called from pollGmailInbox.
    Grep for "integration.email_routed" in gmail.ts to confirm routing results are persisted.
  ...
  <done>
    Gmail polling fetches new messages via historyId tracking, emits integration.email_received events, and fires off AI email routing (analyzeEmailRouting) per message with results stored as integration.email_routed events. Email parser extracts structured data from Gmail API responses. All TypeScript compiles.
  </done>
</task>

<task type="auto">
  <name>Task 2: AI email router, AI email drafter, and integration REST routes</name>
  <files>
    server/src/integrations/email/router.ts
    server/src/integrations/email/drafter.ts
    server/src/routes/integrations.ts
  ...
  <action>
1. Create `server/src/integrations/email/router.ts`:
...
2. Create `server/src/integrations/email/drafter.ts`:
...
3. Create `server/src/routes/integrations.ts`:
   ...
   **Gmail polling control:**
   - POST /gmail/start-polling: Start Gmail polling for the authenticated user (call startGmailPolling(userId, teamId, language)). Return 200.
   ...
   **Email drafts (INTG-05):**
   ...
   - Fire-and-forget: emitNotificationForEvent for email_sent events
  ...
  <done>
    AI email router classifies emails to projects (mirrors voice routing). AI generates email draft replies stored as pending events. Users can list, approve, and send drafts via Gmail API. Integration routes provide OAuth flow, status check, polling control, email listing, routing, and draft management. All TypeScript compiles.
  </done>
</task>
...
- AI email routing is explicitly called from pollGmailInbox (after step e, email_received emission) with results stored as integration.email_routed events
