...
2. **shared/types/models.ts** -- Add at the end of the file:
   - `EntrySourceType` type union: `"text" | "audio" | "decision" | "ai_reflection" | "status_change" | "kpi" | "pattern_match" | "override" | "assumption"`
     NOTE on future integration: The "audio" source type is prepared for Phase 6 (Voice & Idea Pool) when voice input events will be available. The "ai_reflection" source type is prepared for Phase 4 (AI Foundation) when AI analysis events will be available. Both are included now so the type system is forward-compatible and no schema migration is needed later.
...
1. **server/src/docs/writer.ts** -- The DocumentWriter service:
   ...
   - `processEventForDocumentation(event: DomainEvent): void` -- Event listener:
     ...
     Add explicit comments in the EVENT_TO_ENTRY_MAP for future phase integration points:
     `// Phase 6: audio events will be wired here when Voice & Idea Pool is built (maps to sourceType: "audio")`
     `// Phase 4: AI reflection events will be wired here when AI Foundation is built (maps to sourceType: "ai_reflection")`
     These comments document that the "audio" and "ai_reflection" source types in EntrySourceType are intentionally not yet mapped -- they will be wired when their respective phases produce the events.
...
4. **server/src/app.ts** -- Add import for docsRouter and wire it:
  ...
  <done>
    ...
    - EVENT_TO_ENTRY_MAP includes explicit comments for Phase 6 (audio events) and Phase 4 (AI reflection events) future integration points
...
<success_criteria>
The complete backend for Soul Documents is operational: types compile, reducer builds state from events, DocumentWriter creates dual JSONL+Markdown entries, CRUD operations work including GDPR soft-delete with Markdown regeneration, and all API endpoints are wired into Express. The "audio" and "ai_reflection" source types are defined in EntrySourceType for forward-compatibility but not yet mapped in EVENT_TO_ENTRY_MAP (Phase 6 and Phase 4 respectively).
</success_criteria>
