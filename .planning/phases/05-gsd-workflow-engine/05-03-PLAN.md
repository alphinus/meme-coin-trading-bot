...
<task type="auto">
  ...
  <verify>
    Run `npx tsc --noEmit` from project root. Verify app.ts imports workflowRouter. Verify ProjectDetail imports WorkflowEngine. Verify en.json and de.json both contain "workflow" key. Start the dev server (`npm run dev`) and navigate to a project -- the WorkflowEngine should render for the current phase showing the first step.
  </verify>
  <done>
    Workflow route registered at /api/workflow. ProjectDetail renders WorkflowEngine for active phase and PhaseContent for history/archived. PhaseManagement hidden during active workflow. EN and DE translations complete for all workflow steps. Dev server starts without errors.
  </done>
</task>
