...
must_haves:
  ...
  key_links:
    ...
    - from: "client/src/pages/Dashboard.tsx"
      ...
      via: "import and render with projects prop"
      pattern: "ProjectTabs"
    - from: "client/src/pages/Dashboard.tsx"
...
<objective>
Close two verification gaps from Phase 2: (1) implement browser-tab-like project navigation (PROJ-02), and (2) implement pie chart status distribution on the dashboard (PROJ-07).
...
- Accept props: `projects: ProjectState[]` (active projects), `archived: ProjectState[]`, `onSelect: (id: string) => void`
...
- If archived projects exist, show an "Archive (N)" tab at the end, styled differently (muted/gray)
- Tab click calls `onSelect(projectId)` which the parent uses to navigate via `useNavigate`
...
- Pass `projects`, `archived`, and an `onSelect` handler that calls `navigate('/project/${id}')`
...
2. Visually confirm ProjectTabs.tsx exists and exports a function component
3. Dashboard.tsx imports and renders ProjectTabs
4. i18n keys present in both en.json and de.json
  </verify>
  <done>
ProjectTabs component renders active projects as horizontal browser tabs with phase indicators. Tabs scroll horizontally on overflow. Archive tab appears when archived projects exist. Dashboard renders ProjectTabs above the project cards section. All text is translated in both languages.
  </done>
</task>
...
3. StatusPieChart.tsx exists and exports a component
4. Dashboard.tsx imports and conditionally renders StatusPieChart
5. Start the dev server (`npm run dev` from project root) and confirm no runtime errors on the dashboard page
  </verify>
  <done>
...
6. Both components use translated text (switch language to verify)
7. No regressions: existing project cards, team management, and activity feed still work
</verification>
...
- Dashboard.tsx imports and renders both new components
